...
class PuzzleHistoryItemDao {
    ...
    suspend fun getById(id: Int): PuzzleHistoryItemDto? {
        return dbQuery {
            PuzzleHistoryItems.select { PuzzleHistoryItems.id eq id }.map(::from).singleOrNull()
        }
    }

    suspend fun getAllByUserId(userId: Int, limit: Int, skip: Long): List<PuzzleHistoryItemDto> {
        return dbQuery {
            PuzzleHistoryItems.select { PuzzleHistoryItems.userId eq userId }.limit(limit, skip).map(::from)
        }
    }
